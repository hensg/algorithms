...
unsigned long karatsuba_mult(std::string x, std::string y, unsigned long n) {
    if (n == 1)
    ...
    unsigned long ac = karatsuba_mult(a, c, half_n);
    unsigned long ad = karatsuba_mult(a, d, half_n);
    unsigned long bc = karatsuba_mult(b, c, half_n);
    unsigned long bd = karatsuba_mult(b, d, half_n);
